...
def print_metrics(name: str, m: dict):
    ...
    for k in ["accuracy", "precision", "recall", "f1", "roc_auc"]:
        print(f"  {k:<12}: {m[k]:.4f}")
    print(f"  custo_total: R$ {m['custo_total']:,}  "
          f"(FP={m['fp']}, FN={m['fn']})")
...
def main():
    ...
    for name, clf, params in baselines:
        print(name)
        results[name], _ = evaluate_baseline(
            name, clf, X_train, y_train, X_test, y_test, params
        )
    ...
    with mlflow_service.start_run(run_name="MLP_evaluate"):
        mlflow_service.log_metrics({k: v for k, v in mlp_metrics.items() if isinstance(v, (int, float))})
    ...
    summary = pd.DataFrame(results).T[
        ["accuracy", "precision", "recall", "f1", "roc_auc", "custo_total"]
    ...
    for t in np.arange(0.1, 0.9, 0.05):
        ...
        rows.append({"threshold": round(t, 2), "recall": m["recall"],
                     "precision": m["precision"], "custo": m["custo_total"],
                     "fp": m["fp"], "fn": m["fn"]})
